...
const inputClass =
  "w-full rounded-md border border-white/10 bg-ink px-3 py-2 text-sm text-cream placeholder:text-stone/40 focus:border-gold focus:outline-none";
...
const sectionTitleClass = "font-display text-xl text-cream mb-5";
      ...
      <section className={sectionClass}>
        ...
        <div className="space-y-4">
          ...
          <ImagePathField
            ...
            gallery={gallery}
            hint="Leave blank to automatically use the first photo from your gallery."
          />
        </div>
      </section>
      ...
      <section className={sectionClass}>
        ...
        {gallery.length > 0 ? (
          <div className="mt-6 grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4">
            {gallery.map((image) => (
              <div key={image.src} className="group relative aspect-square overflow-hidden rounded-md bg-ink">
                <Image
                  src={image.src}
                  alt={image.name}
                  fill
...
function SaveBar({ status, onSave }: { status: Status; onSave: () => void }) {
  return (
    <div className="flex flex-wrap items-center gap-4 rounded-lg border border-white/10 bg-charcoal p-4">
      <button
        type="button"
        onClick={onSave}
